...
SCHEDULE = ["11:00", "15:00", "16:18", "17:00", "19:00", "21:00"]  # Kyiv
...
def generate_image_bytes(direction, text):
    try:
        ...
        # Some API versions don't accept response_format — default is b64_json
...
def md_to_html(s: str) -> str:
    ...
    # convert double newlines to paragraph breaks (Telegram tolerates \n)
    return s

def send_text(text):
    url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage"
    html_text = md_to_html(text)
    data = {"chat_id": CHANNEL_ID, "text": html_text, "parse_mode": "HTML"}
...
def send_photo_with_caption(image_bytes, caption):
    ...
    data = {"chat_id": CHANNEL_ID, "caption": html_caption, "parse_mode": "HTML"}
    ...
    while True:
        try:
            ...
            if now_hhmm in SCHEDULE and slot_id not in posted_slots:
                with_image = FORCE_IMAGES or (now_hhmm in img_slots)
